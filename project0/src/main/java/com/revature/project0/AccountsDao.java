...
/**
 * Accounts Data Access Object (DAO).
 * This class contains all database handling that is needed to 
 * permanently store and retrieve Accounts object instances. 
 */
...
public class AccountsDao {
	 ...
	 */
	public Accounts createValueObject() {
		return new Accounts();
	}
	 ...
	 */
	public Accounts getObject(Connection conn, int acctID) throws NotFoundException, SQLException {

		Accounts valueObject = createValueObject();
		valueObject.setAcctID(acctID);
	 ...
	 */
	public void load(Connection conn, Accounts valueObject) throws NotFoundException, SQLException {
		...
		try {
			...
			stmt.setInt(1, valueObject.getAcctID());
	 ...
	 */
	public synchronized void create(Connection conn, Accounts valueObject) throws SQLException {
		...
		try {
			sql = "INSERT INTO accounts ( AccountID, TypeOfAccount, UserID, " + "Balance) VALUES (?, ?, ?, ?) ";
			stmt = conn.prepareStatement(sql);

			stmt.setInt(1, valueObject.getAcctID());
			stmt.setString(2, valueObject.getAcctType());
			stmt.setInt(3, valueObject.getUserID());
			stmt.setString(4, valueObject.getFinBal());
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void save(Connection conn, Accounts valueObject) throws NotFoundException, SQLException {

		String sql = "UPDATE accounts SET TypeOfAccount = ?, UserID = ?, Balance = ? WHERE (AccountID = ? ) ";
		PreparedStatement stmt = null;

		try {
			stmt = conn.prepareStatement(sql);
			stmt.setString(1, valueObject.getAcctType());
			stmt.setInt(2, valueObject.getUserID());
			stmt.setString(3, valueObject.getFinBal());

			stmt.setInt(4, valueObject.getAcctID());
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void delete(Connection conn, Accounts valueObject) throws NotFoundException, SQLException {
	 ...
	 */
	public List searchMatching(Connection conn, Accounts valueObject) throws SQLException {

		List searchResults;

		boolean first = true;
		StringBuffer sql = new StringBuffer("SELECT * FROM accounts WHERE 1=1 ");

		if (valueObject.getAcctID() != 0) {
			if (first) {
				first = false;
			}
			sql.append("AND AccountID = ").append(valueObject.getAcctID()).append(" ");
		}

		if (valueObject.getAcctType() != null) {
			if (first) {
				first = false;
			}
			sql.append("AND TypeOfAccount LIKE '").append(valueObject.getAcctType()).append("%' ");
		}
		...
		if (valueObject.getFinBal() != null) {
			if (first) {
				first = false;
			}
			sql.append("AND Balance LIKE '").append(valueObject.getFinBal()).append("%' ");
		}
	 ...
	 */
	protected void singleQuery(Connection conn, PreparedStatement stmt, Accounts valueObject)
		...
		try {
			...
			if (result.next()) {
				...
				valueObject.setAcctID(result.getInt("AccountID"));
				valueObject.setAcctType(result.getString("TypeOfAccount"));
				valueObject.setUserID(result.getInt("UserID"));
				valueObject.setFinBal(result.getString("Balance"));

			} else {
				// System.out.println("Accounts Object Not Found!");
				throw new NotFoundException("Accounts Object Not Found!");
			}
		} finally {
			if (result != null)
				result.close();
	...
	protected List listQuery(Connection conn, PreparedStatement stmt) throws SQLException {
		...
		try {
			...
			while (result.next()) {
				Accounts temp = createValueObject();

				temp.setAcctID(result.getInt("AccountID"));
				temp.setAcctType(result.getString("TypeOfAccount"));
				temp.setUserID(result.getInt("UserID"));
				temp.setFinBal(result.getString("Balance"));
