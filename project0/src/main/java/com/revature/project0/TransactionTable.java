package com.revature.project0;

import java.io.*;
import java.sql.*;
import java.util.*;
import java.math.*;

/**
 * TransactionTable Value Object.
 * This class is value object representing database table TransactionTable
 * This class is intented to be used together with associated Dao object.
 */

/**
 * This sourcecode has been generated by FREE DaoGen generator version 2.4.1.
 * The usage of generated code is restricted to OpenSource software projects
 * only. DaoGen is available in http://titaniclinux.net/daogen/ It has been
 * programmed by Tuomo Lukka, Tuomo.Lukka@iki.fi
 *
 * DaoGen license: The following DaoGen generated source code is licensed under
 * the terms of GNU GPL license. The full text for license is available in GNU
 * project's pages: http://www.gnu.org/copyleft/gpl.html
 *
 * If you wish to use the DaoGen generator to produce code for closed-source
 * commercial applications, you must pay the lisence fee. The price is 5 USD or
 * 5 Eur for each database table, you are generating code for. (That includes
 * unlimited amount of iterations with all supported languages for each database
 * table you are paying for.) Send mail to "Tuomo.Lukka@iki.fi" for more
 * information. Thank you!
 */

public class TransactionTable implements Cloneable, Serializable {

	/**
	 * Persistent Instance variables. This data is directly mapped to the columns of
	 * database table.
	 */
	private int tranID;
	private String acctID;
	private String balTime;
	private String time;
	private String tranAmt;
	private String tranType;

	/**
	 * Constructors. DaoGen generates two constructors by default. The first one
	 * takes no arguments and provides the most simple way to create object
	 * instance. The another one takes one argument, which is the primary key of the
	 * corresponding table.
	 */

	public TransactionTable() {

	}

	public TransactionTable(int tranIDIn) {

		this.tranID = tranIDIn;

	}

	/**
	 * Get- and Set-methods for persistent variables. The default behaviour does not
	 * make any checks against malformed data, so these might require some manual
	 * additions.
	 */

	public int getTranID() {
		return this.tranID;
	}

	public void setTranID(int tranIDIn) {
		this.tranID = tranIDIn;
	}

	public String getAcctID() {
		return this.acctID;
	}

	public void setAcctID(String acctIDIn) {
		this.acctID = acctIDIn;
	}

	public String getBalTime() {
		return this.balTime;
	}

	public void setBalTime(String balTimeIn) {
		this.balTime = balTimeIn;
	}

	public String getTime() {
		return this.time;
	}

	public void setTime(String timeIn) {
		this.time = timeIn;
	}

	public String getTranAmt() {
		return this.tranAmt;
	}

	public void setTranAmt(String tranAmtIn) {
		this.tranAmt = tranAmtIn;
	}

	public String getTranType() {
		return this.tranType;
	}

	public void setTranType(String tranTypeIn) {
		this.tranType = tranTypeIn;
	}

	/**
	 * setAll allows to set all persistent variables in one method call. This is
	 * useful, when all data is available and it is needed to set the initial state
	 * of this object. Note that this method will directly modify instance variales,
	 * without going trough the individual set-methods.
	 */

	public void setAll(int tranIDIn, String acctIDIn, String balTimeIn, String timeIn, String tranAmtIn,
			String tranTypeIn) {
		this.tranID = tranIDIn;
		this.acctID = acctIDIn;
		this.balTime = balTimeIn;
		this.time = timeIn;
		this.tranAmt = tranAmtIn;
		this.tranType = tranTypeIn;
	}

	/**
	 * hasEqualMapping-method will compare two TransactionTable instances and return
	 * true if they contain same values in all persistent instance variables. If
	 * hasEqualMapping returns true, it does not mean the objects are the same
	 * instance. However it does mean that in that moment, they are mapped to the
	 * same row in database.
	 */
	public boolean hasEqualMapping(TransactionTable valueObject) {

		if (valueObject.getTranID() != this.tranID) {
			return (false);
		}
		if (this.acctID == null) {
			if (valueObject.getAcctID() != null)
				return (false);
		} else if (!this.acctID.equals(valueObject.getAcctID())) {
			return (false);
		}
		if (this.balTime == null) {
			if (valueObject.getBalTime() != null)
				return (false);
		} else if (!this.balTime.equals(valueObject.getBalTime())) {
			return (false);
		}
		if (this.time == null) {
			if (valueObject.getTime() != null)
				return (false);
		} else if (!this.time.equals(valueObject.getTime())) {
			return (false);
		}
		if (this.tranAmt == null) {
			if (valueObject.getTranAmt() != null)
				return (false);
		} else if (!this.tranAmt.equals(valueObject.getTranAmt())) {
			return (false);
		}
		if (this.tranType == null) {
			if (valueObject.getTranType() != null)
				return (false);
		} else if (!this.tranType.equals(valueObject.getTranType())) {
			return (false);
		}

		return true;
	}

	/**
	 * toString will return String object representing the state of this
	 * valueObject. This is useful during application development, and possibly when
	 * application is writing object states in textlog.
	 */
	public String toString() {
		StringBuffer out = new StringBuffer(this.getDaogenVersion());
		out.append("\nclass TransactionTable, mapping to table TransactionTable\n");
		out.append("Persistent attributes: \n");
		out.append("tranID = " + this.tranID + "\n");
		out.append("acctID = " + this.acctID + "\n");
		out.append("balTime = " + this.balTime + "\n");
		out.append("time = " + this.time + "\n");
		out.append("tranAmt = " + this.tranAmt + "\n");
		out.append("tranType = " + this.tranType + "\n");
		return out.toString();
	}

	/**
	 * Clone will return identical deep copy of this valueObject. Note, that this
	 * method is different than the clone() which is defined in java.lang.Object.
	 * Here, the retuned cloned object will also have all its attributes cloned.
	 */
	public Object clone() {
		TransactionTable cloned = new TransactionTable();

		cloned.setTranID(this.tranID);
		if (this.acctID != null)
			cloned.setAcctID(new String(this.acctID));
		if (this.balTime != null)
			cloned.setBalTime(new String(this.balTime));
		if (this.time != null)
			cloned.setTime(new String(this.time));
		if (this.tranAmt != null)
			cloned.setTranAmt(new String(this.tranAmt));
		if (this.tranType != null)
			cloned.setTranType(new String(this.tranType));
		return cloned;
	}

	/**
	 * getDaogenVersion will return information about generator which created these
	 * sources.
	 */
	public String getDaogenVersion() {
		return "DaoGen version 2.4.1";
	}

}
