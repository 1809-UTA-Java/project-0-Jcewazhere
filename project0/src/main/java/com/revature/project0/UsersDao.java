 ...
 * permanently store and retrieve Users object instances. 
 */
...
public class UsersDao {
	 ...
	 */
	public Users createValueObject() {
		return new Users();
	}
	 ...
	 */
	public Users getObject(Connection conn, int UserID) throws NotFoundException, SQLException {

		Users valueObject = createValueObject();
	 ...
	 */
	public void load(Connection conn, Users valueObject) throws NotFoundException, SQLException {
	 ...
	 */
	public synchronized void create(Connection conn, Users valueObject) throws SQLException {
		...
		try {
			sql = "INSERT INTO users ( UserID, FName, LName, " + "isActive, zip, DoB, "
					+ "Username, Password) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ";
			stmt = conn.prepareStatement(sql);

			stmt.setInt(1, valueObject.getUserID());
			stmt.setString(2, valueObject.getFName());
			stmt.setString(3, valueObject.getLName());
			stmt.setString(4, valueObject.getIsActive());
			stmt.setInt(5, valueObject.getZip());
			stmt.setString(6, valueObject.getDoB());
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void save(Connection conn, Users valueObject) throws NotFoundException, SQLException {

		String sql = "UPDATE users SET FName = ?, LName = ?, isActive = ?, " + "zip = ?, DoB = ?, Username = ?, "
				+ "Password = ? WHERE (UserID = ? ) ";
		PreparedStatement stmt = null;

		try {
			stmt = conn.prepareStatement(sql);
			stmt.setString(1, valueObject.getFName());
			stmt.setString(2, valueObject.getLName());
			stmt.setString(3, valueObject.getIsActive());
			stmt.setInt(4, valueObject.getZip());
			stmt.setString(5, valueObject.getDoB());
			stmt.setString(6, valueObject.getUsername());
			stmt.setString(7, valueObject.getPassword());

			stmt.setInt(8, valueObject.getUserID());
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void delete(Connection conn, Users valueObject) throws NotFoundException, SQLException {
	 ...
	 */
	public List searchMatching(Connection conn, Users valueObject) throws SQLException {
		...
		if (valueObject.getZip() != 0) {
			if (first) {
				first = false;
			}
			sql.append("AND zip = ").append(valueObject.getZip()).append(" ");
		}
		...
		if (valueObject.getDoB() != null) {
			...
			sql.append("AND DoB LIKE '").append(valueObject.getDoB()).append("%' ");
		}
	 ...
	 */
	protected void singleQuery(Connection conn, PreparedStatement stmt, Users valueObject)
		...
		try {
			...
			if (result.next()) {
				...
				valueObject.setZip(result.getInt("zip"));
				valueObject.setDoB(result.getString("DoB"));
				...
				throw new NotFoundException("Users Object Not Found!");
			}
		} finally {
			if (result != null)
				result.close();
	...
	protected List listQuery(Connection conn, PreparedStatement stmt) throws SQLException {
		...
		try {
			...
			while (result.next()) {
				...
				temp.setZip(result.getInt("zip"));
				temp.setDoB(result.getString("DoB"));
